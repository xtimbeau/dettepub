...
data <- bind_rows(
  ...
  mutate(
    ...
    event = factor(event, c("iran", "twelve", "ukr", "GWI", "GWII")),
    tooltip = glue::glue(
      "day {ifelse(days>0, '+', '')}{days}
...
gg1 <- ggplot(
  data |> filter(com %in% c("brent", "gaz", "fertilizers", "corn"))
) +
...
gg2 <- ggplot(
  data |> filter(com %in% c("euro dollar", "10y bills", "gold", "dow"))
) +
  aes(x = days, y = (open_r + close_r) / 2, color = event, group = event) +
  geom_line() +
  geom_pointrange_interactive(
